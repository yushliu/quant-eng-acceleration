...
def ewma_covariance(
    ...
    s_t = np.cov(base, rowvar=False, ddof=1).astype(np.float64, copy=False)
    if s_t.shape == ():
    ...
    s_t = 0.5 * (s_t + s_t.T)
    s_t = s_t + np.float64(jitter_eps) * np.eye(n_assets, dtype=np.float64)
    ...
    return np.asarray(s_t, dtype=np.float64)
