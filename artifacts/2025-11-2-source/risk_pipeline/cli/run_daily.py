...
def run_pipeline(args: argparse.Namespace) -> Path:
    ...
    weights_mode = "equal" if args.weights is None else "custom"

    cache_dir_arg = Path(args.cache_dir) if args.cache_dir else (root / "data" / "cache" / "yfinance")
    use_download_patch = enable_download_patch or len(tickers) > 1
    ...
    if use_download_patch:
        prices, cache_metadata, cache_dir, download_report = download_prices_chunked(
            tickers=tickers,
        ...
        download_report = {
            ...
            "summary": {
                ...
                "succeeded": 1,
                "failed": 0,
                "cache_hits": int(bool(cache_metadata.get("cache_hit", False))),
                "total_retries": 0,
            },
        }
    ...
    portfolio_returns = returns_df @ weights_arr
    portfolio_returns_df = portfolio_returns.to_frame(name="PORTFOLIO")
    portfolio_returns = returns_to_numpy(portfolio_returns_df)
    risk_weights = np.asarray([1.0], dtype=float)
    ...
    params_payload = {
        ...
        "args": {
            ...
            "enable_download_patch": use_download_patch,
            "chunk_months": args.chunk_months,
            "cache_dir": str(cache_dir_arg),
            "download_retries": args.download_retries,
            "download_base_sleep": args.download_base_sleep,
